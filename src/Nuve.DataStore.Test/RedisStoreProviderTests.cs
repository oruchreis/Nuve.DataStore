...
public class RedisStoreProviderTests
{
    ...
    public void LockSlidingExpiration()
    {
        ...
        var lockItem = provider.AcquireLock("test-lock", TimeSpan.FromSeconds(1), throwWhenTimeout: true, slidingExpire: slidingExpire)!;
        try
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                Thread.Sleep(Math.Max((int)ttl.Value.TotalMilliseconds - ((int)slidingExpire.TotalMilliseconds / 2) + RedisLock.CheckSlidingExpirationMs + 1000, 1));
