    ...
    public sealed class HashStore: DataStoreBase
    {
            ...
            base(connectionName, defaultExpire, autoPing, namespaceSeperator, overrideRootNamespace, serializer, profiler, compressBiggerThan)
        {
            ...
            if (_dictionaryStoreProvider == null)
                throw new InvalidOperationException(string.Format("The provider with connection '{0}' doesn't support Dictionary operations. " +
                                                                  "The provider must implement IDictionaryStoreProvider interface to use HashStore", connectionName));
